...
const ReceiptPage = () => {
  ...
  const handleSubmitReceipt = async () => {
    try {
      ...
      const payload = {
        Source: "MCollect",
        UniqueId: Date.now(), // Generate unique ID
        LegalEntity: "Tvs Automobile solutions",
        BusinessUnit: "DELPHI",
        CustomerId: selectedCustomer.id || selectedCustomer.customer_id,
        CustomerNumber: selectedCustomer.code || selectedCustomer.customer_code,
        CustomerSiteId: selectedCustomer.site_id || 100000003393354,
        CustomerSiteCode: selectedCustomer.site_code || selectedCustomer.code,
        CustomerName: selectedCustomer.name || selectedCustomer.customer_name,
        ReceiptMethod: selectedMethod,
        UTRNumber: formData.utrNumber || Math.floor(Math.random() * 1000000000),
        ReceiptRefNum: `MC${Date.now()}`,
        ReceiptDate: formData.receiptDate,
        ReceiptAmount: parseFloat(formData.receiptAmount),
        ReceiptCurrency: "INR",
        OrderNumber: "",
        Bank: formData.bankName || (selectedMethod === 'Cash' ? 'Cash' : 'HDFC'),
        ReceiptMode: "Batch",
        ...
        CreatedBy: localStorage.getItem('userName') || "Sales Executive",
        LastUpdatedBy: localStorage.getItem('userName') || "Sales Executive",
        ProcessFlag: "",
        ProcessMessage: "",
        ChequeNo: formData.chequeNumber || null,
        DraweeBank: formData.bankName || "",
        PaymentFor: "",
        ChallanDdChequeNumber: formData.chequeNumber || "",
        ChallanDdChequeDate: formData.challanDdChequeDate || formData.receiptDate,
      ...
      const response = await apiService.post('/receipt/create-receipt', payload);
      ...
      alert(err.response?.data?.message || 'Failed to create receipt. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
      ...
      <div className="receipt-page-container">
        <div className="receipt-content">
          ...
          {view === 'list' && <ListView customers={customers} setView={setView} setSelectedCustomer={setSelectedCustomer} loading={loading} error={error} />}
...
const ListView = ({ customers, setView, setSelectedCustomer, loading, error }) => (
  <div className="view-container">
    {loading ? (
      ...
      <div className="list-wrapper">
        {customers.map((c, i) => (
          <div key={c.id || i} className="customer-card" onClick={() => { setSelectedCustomer(c); setView('orders'); }}>
