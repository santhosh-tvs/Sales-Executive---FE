...
const VisitReport = () => {
  ...
  const handleExport = () => {
    const exportData = filtered.map((row, idx) => ({
      "S.No": idx + 1,
      "Visited Date": formatDateTime(row.check_in),
      "Employee Code": row.employee_code || "-",
      "Employee Name": row.employee_name || "-",
      "Visited Location": row.checkin_lat_long || "-",
      "Customer Name": row.garage_name || "-",
      "Customer Location": row.garage_location || "-",
      "Feedback Date Time": formatDateTime(row.check_out),
      "Visited Remarks": row.plan_remarks || "-",
    }));
    ...
    saveAs(new Blob([buf], { type: "application/octet-stream" }),
      `Visit_Report_${new Date().toISOString().slice(0, 10)}.xlsx`);
  };
        ...
        <div className="vr-content">
          ...
          <div className="vr-table-wrapper">
            <table className="vr-table">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={9} className="vr-empty"><Spinner size="sm" text="Loading..." /></td></tr>
                ) : currentData.length === 0 ? (
                  <tr><td colSpan={9} className="vr-empty">No records found</td></tr>
